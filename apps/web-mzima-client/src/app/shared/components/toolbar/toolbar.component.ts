...
export class ToolbarComponent extends BaseComponent implements OnInit {
  ...
  public searchPosts(query?: string): void {
  ...
  public changeNorificationStatus(event: MatSlideToggleChange): void {
    let set: string | undefined = undefined;
    if (this.collectionInfo) {
      set = this.collectionInfo?.id?.toString();
    }
    if (this.activeSavedSearch?.id) {
      set = this.activeSavedSearch?.id?.toString();
    }
