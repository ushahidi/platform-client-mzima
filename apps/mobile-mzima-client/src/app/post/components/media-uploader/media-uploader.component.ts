  ...
  async clickDeleteButton(value: number | undefined, generatedId: number) {
    ...
    if (mediaFile) {
      if (
        ...
        const confirmed = await this.alertService.presentAlert({
          header: this.translate.instant('notify.default.are_you_sure_you_want_to_delete_this'),
          message: this.translate.instant('notify.default.proceed_warning'),
          buttons: [
            {
              text: 'Cancel',
              role: 'cancel',
        ...
        if (confirmed.role === 'cancel') return;
